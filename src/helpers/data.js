import { agentData, customerData, customerReviewsData, dataTableRecords, pricingData, projectsData, propertyData, timelineData, userData, transactionData } from '@/assets/data/other';
import { sellersData } from '@/assets/data/product';
import { tradesData }  from '@/assets/tychiData/data';
...
export const getAllTrades = async () => {
  const data = tradesData.map(item => {
    const user = userData.find(user => user.id === item.userId);
    const property = propertyData.find(property => property.id == item.propertyId);
    return {
      ...item,
