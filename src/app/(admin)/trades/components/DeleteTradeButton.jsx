...
const DeleteTradeButton = ({ data, api, canDelete }) => {
  ...
  const handleDelete = async () => {
    ...
    if (!window.confirm(`Delete trade ${data.trade_id}?`)) return

    try {
      setLoading(true)
      const token = Cookies.get('dashboardToken')

      await axios.delete(`/api/v1/trade/${data.trade_id}`, {
      ...
      if (api) api.applyTransaction({ remove: [data] })
